...
const reducer=(state,action)=>{
  ...
  switch(action.type){
    case "add-item":{
      // use of id to add quantity, see in console
      const isExist = state.cartItems.find(
        (item)=>item.id == action.payload.id);

        if(isExist)
        {
          const updatedCart = state.cartItems.map((item)=> item.id == action.payload.id?
    ...
    case "decrement":{
      return {...state, cartItems:state.cartItems.map((item)=> item.id == action.payload? {...item, quantity:item.quantity>1? item.quantity-1: item.quantity}: item)}
    }
